...
export default function RegisterCommunityScreen() {
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              await deleteCommunity(params.id as string);
              
              CustomAlert.alert(
                "Comunidade Excluída",
                "A comunidade foi excluída com sucesso!",
                [
                  {
                    text: "OK",
              ...
              CustomAlert.alert(
                "Erro",
                "Não foi possível excluir a comunidade. Tente novamente.",
                [{ text: "OK" }]
              );
            } finally {
  ...
  }, [isEditMode, colorScheme, navigation, router, params.id]);
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (data.imageUrl && !data.imageUrl.startsWith('http')) {
        ...
        imageUrl = await uploadCommunityImage(data.imageUrl, fileName);
      }
        ...
        [
          {
            text: "OK",
            onPress: () => {
              // Navigate back to the communities tab with a refresh parameter
              router.replace({
                pathname: '/(tabs)/communities',
                params: { refresh: Date.now().toString() }
              });
